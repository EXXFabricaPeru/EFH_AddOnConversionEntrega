...
namespace AddOnUpdPrice.App
{
    public class Setup
    {
        ...
        public void confirmarVersion(string addOnName, string addOnVersion)
        {
            try
            {
                Globals.Query = AddOnUpdPrice.Properties.Resources.DeleteSetup + "'" + addOnName + "'";
                Globals.RunQuery(Globals.Query);
                Globals.Release(Globals.oRec);
                Globals.Query = AddOnUpdPrice.Properties.Resources.HanaInsertSetup + "(" + getCorrelativo("Code", "[@EXX_SETUP]", "", 1000) + ", '" + getCorrelativo("Code", "[@EXX_SETUP]", "", 1000) + "', '" + addOnName + "','" + addOnVersion + "','" + "')";
                Globals.RunQuery(Globals.Query);
                Globals.Release(Globals.oRec);
            }
            catch (Exception ex)
        ...
        public string getCorrelativo(string CampoMax, string Tabla, string condicion = "", int primerCorrelativo = 1)
        {
            ...
            try
            {
                ...
                if (Globals.IsHana() == true)
                    Srt = AddOnUpdPrice.Properties.Resources.HanaCorrInstall;
                if (Globals.IsHana() == false)
                    Srt = AddOnUpdPrice.Properties.Resources.SQLCorrInstall;
